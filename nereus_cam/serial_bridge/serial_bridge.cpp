...
/*!
 ...
 @return true always
 */
static bool tx_cb(const uint8_t *buff, size_t len,
                  bm_serial_message_t message) {
  (void)message;
 ...
 */
static void sub_message_cb(uint64_t node_id, const char *topic,
                           uint16_t topic_len, const uint8_t *data,
                           uint16_t data_len, uint8_t type, uint8_t version) {
  if (bm_serial_pub(node_id, topic, topic_len, data, data_len, type, version) !=
      BM_SERIAL_OK) {
...
void serial_bridge_handle(void) {
  while (PLUART::byteAvailable()) {
    ...
    if (ctx.rx.idx >= RX_BUF_SIZE) {
      ...
      if (b == 0) {
        cobs_decode_result result = cobs_decode(ctx.packet.buffer, RX_BUF_SIZE,
                                                ctx.rx.buffer, ctx.rx.idx - 1);
        if (result.status == COBS_DECODE_OK) {
          bm_serial_packet_t *packet =
              reinterpret_cast<bm_serial_packet_t *>(ctx.packet.buffer);
          bm_serial_error_e err =
              bm_serial_process_packet(packet, result.out_len);
          if (err != BM_SERIAL_OK) {
